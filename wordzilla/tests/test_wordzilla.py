import argparse
import unittest
from wordzilla import check_word, get_meanings, parser, db

class WordzillaTest(unittest.TestCase):

    def setUp(self):
        self.all_pronouns_meanings_we = filter(lambda part: part[0] == 'Pronoun' , get_meanings('we', 'pronoun'))
        self.all_noun_meanings_we = filter(lambda part: part[0] == 'Noun' , get_meanings('we', 'pronoun'))
    ...
    def test_get_meanings(self):
        ...
        self.failUnlessEqual(len(list(get_meanings())), 1)
...
class WordzillaOptionParseTest(unittest.TestCase):
    ...
    def test_option_parser(self):
        ...
        options = parser.parse_args(self.commandline_args_with_number_of_meanings)
        self.failUnlessEqual(options.number_of_meanings, 10)

        options = parser.parse_args(self.commandline_args_without_number_of_meanings)
        ...
        options = parser.parse_args(self.commandline_args_without_part_of_speech)
